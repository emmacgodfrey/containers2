...
class BinaryTree():
    ...
    def to_list(self, traversal_type):
        ...
        should return its results rather than print them whenever possible.
        ...
        functions rely on this to_list function, so it is import
        to implement it correctly.

        FIXME:
        Implement this function by modifying the _print functions above.

        clean = self.print_tree(traversal_type).replace('-','')
        clean_ints = [int(item) for item in clean]
        return clean_ints
        '''
        if traversal_type == 'preorder':
    ...
    def __len__helper(node):
        ...
        if node:
            ...
            result2 = BinaryTree.__len__helper(node.right)
